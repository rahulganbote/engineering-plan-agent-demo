...
> A demo of **EM Copilot** — a production-grade, 7-Agents LangGraph system, grounded with RAG that transforms a raw Business Requirements Document (BRD) into an audit-ready engineering bundle: structured plan, project schedule, Kroki-rendered architecture diagram, PoC definition, and tech stack options. Outputs are evaluated by a Critic Agent, reviewed via a Human-in-the-Loop (HITL) gate supporting voice commands, and deployed to Jira Cloud, Dashboard in Google Sheets , and downloadable PDF.
...
**1. Token limits & Vibe coding** — Running out of tokens quickly when building with AI assistance. Solution: created a day-by-day implementation plan scoped to one feature at a time. Vibe coding works, but only with a structured plan behind it.

**2. Data strategy & Golden datasets** — Output quality is only as good as the input contracts. Lesson: define Pydantic schemas for variety of BRD formats, standardize BRD structure at the org level, and maintain versioning of a Golden dataset.

**3. Modular vs. Functional design** — Vibe coding tends to miss extensibility. Refactoring in mid-project was expensive. Lesson: Give the AI your architectural vision and extensibility requirements explicitly, not just the task or feature spec.

**4. Reliable AI** — Less Bells and whistles but more reliable AI will give more adoption. Early versions had more integrations but lower output reliability. Cutting scope to harden the Critic, Eval methods, and security layer produced a more trustworthy system.

**5. Trust through determinism** — LLM judges are optimistic by default. More deterministic Eval methods were added specifically because the Critic was passing outputs it shouldn't have. Lesson: Added 3 deterministic rules in Eval framework.

**6. Hallucination Detection** — Requires active enforcements & guardrails. Asking agents to "cite sources" for grounding is not enough. The Critic must verify the citation maps to a real vector chunk key. Lesson: Design citation verification into the evaluation.

**7. Observability from Day 1** — LangSmith was added mid-project and immediately surfaced latency issues and prompt failures that were invisible before. Lesson: You can't fix what you can't see. LangSmith is powerful for tracing, cost monitoring, and debugging; worth the cost though.

**8. Conversational AI & HITL** — ElevenLabs is easy to configure. Getting the Voice Agent to correctly interpret artifact summaries required to pass structured context into the pipeline is  harder. Troubleshooting voice webhook failures is tricky. Lesson: budget more time for voice integration.

**9. Autonomous vs. Deterministic** — Requires Mindset shift. Let Agent make autonomous call to tool or external system but always keep a human or deterministic check in the loop before irreversible actions (Jira write, Sheets export).

**10. Latency is Product problem not just an engineering one** — 50 seconds felt fast during development. Users expect faster. Lessons: Optimize the whole workflow, optimize LLM response & output. Use Prompt+Semantic Caching. Parallel dispatch was the biggest single win (~3× speedup).

---

## Engineering Highlights

- **7-Agent LangGraph pipeline** with parallel specialist dispatch via `ThreadPoolExecutor` — ~3× faster than sequential chaining (~50s vs >2.5 min)
...
LangGraph · GPT-4o · GPT-4o-mini · Pinecone · LangSmith · FastAPI · Streamlit · ElevenLabs · Jira MCP (mcp-atlassian) · Google Sheets (gspread) · ReportLab · Kroki · Pydantic · BERTScore · tenacity
